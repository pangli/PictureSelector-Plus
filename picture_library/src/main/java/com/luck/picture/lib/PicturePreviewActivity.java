...
public class PicturePreviewActivity extends PictureBaseActivity implements
    ...
    private TextView tv_editor, tv_img_num, tv_title, tv_ok;
    ...
    public void onClick(View view) {
        ...
        if (id == R.id.tv_editor) {
            if (!images.isEmpty() && images.size() - 1 >= position) {
                LocalMedia localMedia = images.get(position);
                Intent intent = new Intent(mContext, PhotoEditorActivity.class);
                intent.putExtra(PhotoEditorActivity.TYPE, PhotoEditorActivity.TYPE_ALBUM);
                intent.putExtra(PhotoEditorActivity.EXTRA_EDITOR_MEDIA, localMedia);
                startActivityForResult(intent, PhotoEditorActivity.REQUESTCODE);
            }
        }
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        } else if (resultCode == PhotoEditorActivity.RESULTCODE) {
            if (requestCode == PhotoEditorActivity.REQUESTCODE) {
                if (data != null) {
                    LocalMedia localMedia = data.getParcelableExtra(PhotoEditorActivity.RESULT_EDITOR_MEDIA);
                    adapter.notifyChangedItem(localMedia);
                    if (!selectImages.isEmpty()) {
                        int position = localMedia.getPosition();
                        for (int i = 0; i < selectImages.size(); i++) {
                            LocalMedia oldLocalMedia = selectImages.get(i);
                            if (position == oldLocalMedia.getPosition()) {
                                selectImages.set(i, localMedia);
                                break;
                            }
                        }
                    }
